...
CREATE TABLE orders(
    ...
    orderdate DATE,
    -- 왜래키 설정
    CONSTRAINT cus_fk FOREIGN KEY(custid) REFERENCES customer(custid),
...
--INSERT INTO orders ALUE
SELECT * from orders;
-- 고객이 주문한 도서의 총 판매액을 구하시오
SELECT SUM(saleprice) 총판매액, AVG(saleprice) 평균금액 FROM orders;
...
SELECT cus.name, ord.saleprice, ord.orderdate FROM orders ord , customer cus WHERE ord.custid = cus.custid AND cus.name = '김연아';

-- 고객이름별로 주문한 도서의 총 판매액을 구하시오
-- 집계를 구할때는 GROUP BY절 사용
-- HAVING 절은 _ GROUP BY의 질의 결과 나타내는 그룹을 제한하는 역할
SELECT cus.name, SUM(ord.saleprice) FROM orders ord , customer cus WHERE ord.custid = cus.custid GROUP BY cus.name HAVING cus.name = '김연아';
