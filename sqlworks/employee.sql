...
SELECT * FROM employee WHERE gender = '남자' AND sal >= 2500000;

-- 급여가 200만원 이상 300만원 이하인 사원을 검색하시오
SELECT * FROM employee WHERE sal BETWEEN 2000000 AND 3000000;
...
-- 부서별 급여 총액을 구하시오
SELECT deptno 부서 , SUM(sal) 급여총액, AVG(sal) 급여평균 FROM employee
GROUP BY deptno HAVING AVG(sal) >= 3000000 ORDER BY AVG(sal) DESC;
